...
# Copy the file containing the required packages to install
COPY ./.src/requirements.apt /tmp/requirements.apt
# Run the install script
RUN /usr/local/bin/install_pkgs.sh /tmp/requirements.apt 
...
# Install nvm, Node.js, and npm packages using nvm
RUN curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.1/install.sh | bash && \
    ...
    npm install -g pajv gitmoji-cli node-red && \
    cp $(which node) /opt/tools/node && \
    cp $(which pajv) /opt/tools/pajv && \
    cp $(which gitmoji) /opt/tools/gitmoji && \
